...
```{r NFE2_Gata1_Meg_peak_addition, echo=FALSE}
#In this version of the analysis I will not do the analysis from called peaks from the data but from a merged NFE2 and Gata1 peak list from Weiss (GSM923586) and Shivdasani
#In command line:
# mergePeaks NFE2_MKD5.RBC2D.MKD5_unique.txt Gata1_GSM923586_PSU_ChipSeq_Megakaryo_GATA1_peaks.txt > Meg_uniquw.NFE2_Gata1.merged.txt
# pos2bed.pl Meg_unique.NFE2_Gata1.merged.txt > Meg_unique.NFE2_Gata1.merged.bed
# 9070 peaks in total.

x.sp3.nfe2_gata1_peaks <- x.sp3.sample_peaks
rmPmatFromSnap(x.sp3.nfe2_gata1_peaks)
# assuming the bed file in the current folder are generated from the clusters


setwd("/mnt/data/common/jonas/scatac/191204_yuan_FLvsBM_HSC_BHHTF7DRXX/snap_atac_analysis/NFE2_Gata1_Meg_ChIP_analysis/")
...
x.sp3.nfe2_gata1_peaks = createPmat(x.sp3.nfe2_gata1_peaks, peak.gr,  do.par = TRUE, num.cores = 4)
x.sp3.nfe2_gata1_peaks = makeBinary(x.sp3.nfe2_gata1_peaks, mat="pmat")

```

```{r pseudo_time_analysis, echo=FALSE}

#Export as a single cell object object. Use same eigs dims as above. This does not work well because very little data is transfered to the seurat object. Can I create a single cell experiment-oject from the pmat and start there? 
#Convert pmat to a matrix.

peak.matrix <- as(object = x.sp3.nfe2_gata1_peaks@pmat, Class = 'matrix')
peak.dataframe <- as.data.frame(t(peak.matrix))

#assign column names corresponding to the cell barcodes.
colnames(peak.dataframe) <- x.sp3.nfe2_gata1_peaks@sample
...
sce$snap_cluster <- x.sp3.nfe2_gata1_peaks@cluster

#Run PCA with scater package.
sce <- runPCA(sce, ncomponents = 20, ntop = 9069)
...
sce2<- runUMAP(sce2 )
sce2$cell_type <- x.sp3.nfe2_gata1_peaks@sample
sce2$snap_cluster <- x.sp3.nfe2_gata1_peaks@cluster
...
ggplot(as.data.frame(colData(sce2)), aes(x = sce2$slingPseudotime_1, y = cell_type, 
    ...
    ggtitle("Cells ordered by Slingshot pseudotime nfe2_gata1_peaks")

ggsave(filename = "slingshot_pseudotime_order_cell_type_nfe2_gata1_peaks.pdf", device = "pdf",path = "/mnt/data/common/jonas/scatac/191204_yuan_FLvsBM_HSC_BHHTF7DRXX/snap_atac_analysis/NFE2_Gata1_Meg_ChIP_analysis/", units = c("cm"), width = 20, height = 15)
...
ggsave(filename = "slingshot_pseudotime_order_snap_cluster.pdf", device = "pdf",path = "/mnt/data/common/jonas/scatac/191204_yuan_FLvsBM_HSC_BHHTF7DRXX/snap_atac_analysis/NFE2_Gata1_Meg_ChIP_analysis/", units = c("cm"), width = 20, height = 15)
...
x.sp3.sample_peaks = makeBinary(x.sp3.sample_peaks, "pmat");

#Perform this on differential peaks only,
#Must subset the pmat to do this:
x.sp3.sample_diff_peaks <- x.sp3.sample_peaks
#Remove old peak matrix.
rmPmatFromSnap(x.sp3.sample_diff_peaks)

#Add in the differential peaks over all the cells.
x.sp3.sample_diff_peaks@pmat <- x.sp3.sample_peaks@pmat[, c(idy.ls[[1]],idy.ls[[2]],idy.ls[[3]],idy.ls[[4]],idy.ls[[5]],idy.ls[[6]])]
#This does not seem to look better so abandon the differential peaks for now.


#To get a complete list. Run this against both mouse and human and combine them in some way.
x.sp3.sample_peaks@mmat = runChromVAR(
...
pseudotime_order$cluster <- x.sp3.nfe2_gata1_peaks@cluster[pseudotime_order$index]
pseudotime_order$sample <- x.sp3.nfe2_gata1_peaks@sample[pseudotime_order$index]
...
pheatmap(motif_matrix_top100[,c(pseudotime_order$index)], cluster_cols = F ,annotation_col = pseudotime_order[,c(1:2,4)], show_rownames=T, show_colnames = F,clustering_distance_rows = "correlation", clustering_method = "ward.D2", annotation_colors = ann_colors, sscale = "column")
