...
#This is done so the MkP cluster will not drench the signal for some genes.
########## Test to cluster the data without the MkPs


which(x.sp2@sample =="CD41neg")
which(x.sp2@sample =="CD41pos")
x.sp3 = x.sp2[c( which(x.sp2@sample =="CD41neg"),  which(x.sp2@sample =="CD41pos")), ];

x.sp3 <- addBmatToSnap(x.sp3)
x.sp3 = makeBinary(x.sp3, mat="bmat")
x.sp3 = runDiffusionMaps(obj=x.sp3, input.mat="bmat", num.eigs=50)

plotDimReductPW(obj=x.sp3, eigs.dims=1:50,point.size=0.3, point.color="grey", point.shape=19,point.alpha=0.6,  pdf.file.name=NULL,  pdf.height=7, pdf.width=7)
# A little tricky but choose the 6 first dimensions since that is a little blob shaped after.

#Graph based clusterning
x.sp3 = runKNN(obj=x.sp3,eigs.dims=1:6,k=50) #We set it to 50 to make a more stringent clustering. More similar neigbours,
x.sp3=runCluster(obj=x.sp3,tmp.folder=tempdir(),louvain.lib="R-igraph",seed.use=10);
x.sp3@metaData$cluster = x.sp3@cluster


#Visualization
x.sp3 = runViz(obj=x.sp3, tmp.folder=tempdir(),dims=2, eigs.dims=1:6, method="umap",seed.use=10)

par(mfrow = c(2, 2))

#Some cluster and qc-plotting.
plotViz(obj=x.sp3,method="umap",  main="CD41negvsCD41pos", point.color=x.sp3@sample,  point.size=0.2, point.shape=19, point.alpha=0.8, text.add=TRUE,text.size=1.5, text.color="black", text.halo.add=TRUE, text.halo.color="white",text.halo.width=0.2,legend.add=FALSE)

plotViz(obj=x.sp3,method="umap",  main="CD41negvsCD41pos", point.color=x.sp3@cluster,  point.size=0.2, point.shape=19, point.alpha=0.8, text.add=TRUE,text.size=1.5, text.color="black", text.halo.add=TRUE, text.halo.color="white",text.halo.width=0.2,legend.add=FALSE)

CD41negvsCD41pos_sample_umap
...
```{r heretical_clustering, echo=FALSE}
#Pool 
...
plotViz(
    obj=x.sp3,
    method="umap", 
    main="CD41neg vs CD41pos cluster",
...
peaks.ls = mclapply(seq(clusters.sel.peaks), function(i){
    ...
    runMACS(
        ...
        output.prefix=paste0("CD41negvsCD41pos_peaks_cluster.", gsub(" ", "_", clusters.sel.peaks)[i]),
...
peaks.sample = lapply(seq(samples.sel.peaks), function(i){
    print(samples.sel.peaks[i]);
    runMACS(
        obj=x.sp3[which(x.sp3@sample==samples.sel.peaks[i]),], 
        output.prefix=paste0("CD41negvsCD41pos_peaks_sample.", gsub(" ", "_", samples.sel.peaks)[i]),
        path.to.snaptools="/opt/anaconda3/bin/snaptools",
        path.to.macs="/opt/anaconda3/bin/macs2",
        gsize="mm", 
        buffer.size=500, 
...
setwd("/mnt/data/common/jonas/scatac/191204_yuan_FLvsBM_HSC_BHHTF7DRXX/snap_atac_analysis/CD41negvsCD41pos/CD41negvsCD41pos_peaks_cluster")
...
x.sp3.sample_peaks = createPmat(x.sp3, peak.gr,  do.par = TRUE, num.cores = 3)
...
library(scater)
sce <- runPCA(sce, ncomponents = 50, ntop = 10000)
sce <- runUMAP(sce,  ntop = 10000)
sce <- runDiffusionMap(sce, ntop = 10000)
save.image("/mnt/data/common/jonas/scatac/191204_yuan_FLvsBM_HSC_BHHTF7DRXX/snap_atac_analysis/CD41negvsCD41pos/CD41negvsCD41pos_snapatac_analyses.RData")
...
########## I will for trajectory analysis use the umap based without PCA reduction fitst since this looks better.
...
sce <- slingshot(sce, reducedDim = "UMAP", clusterLabels = c(rep(1,305), rep(2,552), rep(3,608), rep(4,365), rep(5,398), rep(6,238), rep(7,340), rep(8,232), rep(9,442)))
...
ggplot(as.data.frame(colData(sce)), aes(x = sce$slingPseudotime_1, y = cell_type, 
                              colour = cell_type)) +
    geom_quasirandom(groupOnX = FALSE) +
    theme_classic() +
    xlab("Slingshot pseudotime 1") + ylab("Timepoint") +
    ggtitle("Cells ordered by Slingshot_1 pseudotime var_top10000_peaks")

ggsave(filename = "slingshot_pseudotime_order_1_cell_type_var_top10000_peaks.pdf", device = "pdf",path = "/mnt/data/common/jonas/scatac/191204_yuan_FLvsBM_HSC_BHHTF7DRXX/snap_atac_analysis/CD41negvsCD41pos/", units = c("cm"), width = 20, height = 15)

ggplot(as.data.frame(colData(sce)), aes(x = sce$slingPseudotime_2, y = snap_cluster, 
                              colour = snap_cluster)) +
    geom_quasirandom(groupOnX = FALSE) +
    theme_classic() +
    xlab("Slingshot pseudotime 2") + ylab("Timepoint") +
    ggtitle("Cells ordered by Slingshot pseudotime 2 var_top10000_peaks")

ggsave(filename = "slingshot_pseudotime_order_2_snap_cluster_var_top10000_peaks.pdf", device = "pdf",path = "/mnt/data/common/jonas/scatac/191204_yuan_FLvsBM_HSC_BHHTF7DRXX/snap_atac_analysis/CD41negvsCD41pos/", units = c("cm"), width = 20, height = 15)
...
sample.in.cluster <- sample.in.cluster[,c(4,6)]

#colnames(sample.in.cluster) <- c("FL_HSC","CD41neg", "CD41pos", "MkP")
colnames(sample.in.cluster) <- c("CD41neg","CD41pos")
rownames(sample.in.cluster) <- c("cluster1","cluster2","cluster3","cluster4","cluster5","cluster6","cluster7","cluster8","cluster9")
...
sample.in.cluster$cluster <- factor(sample.in.cluster$cluster, levels = c("cluster1","cluster2","cluster3","cluster4","cluster5","cluster6","cluster7","cluster8","cluster9","cluster10"))

ggplot(sample.in.cluster, aes(variable,value)) + geom_bar(position="fill", stat="identity", aes(fill = cluster)) + theme_bw()


ggsave(filename = "CD41negvsCD41pos_snap_atac_clusters_per_sample.pdf", device = "pdf",path = "/mnt/data/common/jonas/scatac/191204_yuan_FLvsBM_HSC_BHHTF7DRXX/snap_atac_analysis/CD41negvsCD41pos/", units = c("cm"), width = 20, height = 15)
...
ggsave(filename = "CD41negvsCD41pos_snap_atac_samples_per_cluster.pdf", device = "pdf",path = "/mnt/data/common/jonas/scatac/191204_yuan_FLvsBM_HSC_BHHTF7DRXX/snap_atac_analysis/FLvsCD41neg_analysis/", units = c("cm"), width = 20, height = 15)

```

```{r DAR_identification, echo=FALSE}

#Not much sense in basing this analysis on samples so loop over all the clusters
idy.ls = lapply(levels(x.sp3.sample_peaks@cluster), function(cluster_i){
	DARs = findDAR(
...
#Essentially 0 differential peaks with a classical analysis.

names(idy.ls) = levels(x.sp3.sample_peaks@cluster);

par(mfrow = c(3, 3));

for(cluster_i in levels(x.sp3.sample_peaks@cluster)){
	print(cluster_i)
...
```{r homer_motif_analysis, echo=FALSE}


#I need to export the peaks manually and perform a Homer motif analysis separately.
for(cluster_i in levels(x.sp3.sample_peaks@cluster)){
	print(cluster_i)
  
  peak_name <- paste(cluster_i,"CD41negvsCD41pos_DARs.bed", sep = "_")
  
	write.table(x =  as.data.frame( x.sp3.sample_peaks@peak[idy.ls[[cluster_i]],])[,1:3], file = peak_name, sep = "\t", quote = F, row.names = F, col.names = F)


}

#Run batchFindMotifsGenome
#batchFindMotifsGenome.pl mm10 -size given -cpu 6 -p 8 -f *.bed
```
...
x.sp3.sample_peaks_mouse <- x.sp3.sample_peaks

x.sp3.sample_peaks_mouse@mmat = runChromVAR(
    obj=x.sp3.sample_peaks_mouse,
    input.mat="pmat",
...
x.sp3.sample_peaks_human <- x.sp3.sample_peaks

x.sp3.sample_peaks_human@mmat = runChromVAR(
    obj=x.sp3.sample_peaks_human,
    input.mat="pmat",
    genome=BSgenome.Mmusculus.UCSC.mm10,
    min.count=10,
...
x.sp3.sample_peaks_mmat_combined@mmat <- cbind(x.sp3.sample_peaks_mouse@mmat,x.sp3.sample_peaks_human@mmat)
rm(x.sp3.sample_peaks_mouse)
rm(x.sp3.sample_peaks_human)

#Get the names of the motifs
colnames(x.sp3.sample_peaks_mmat_combined@mmat)

plot_order_clusters <- factor( c(rep(c(1), times=305) ,rep( c(2), times=552 ), rep( c(3), times=608 ),rep( c(4), times=365 ),rep( c(5), times=398 ),rep( c(6), times=238 ),rep( c(7), times=340 ),rep(c(8), times=232),rep(c(9), times=442)), levels = c(1:9)  ) 
...
for (i in 1:length(colnames(x.sp3.sample_peaks_mmat_combined@mmat)) ) {
  ...
  dat = data.frame(x=plot_order_clusters, y=x.sp3.sample_peaks_mmat_combined@mmat[,motif_i]);
  ...
  ggsave(filename = paste(motif_i,"CD41negvsCD41pos_chromVAR_enrichment_per_cluster.pdf",sep = "."), device = "pdf", dpi = 150, units = c("cm"), width = 10, height = 9)
}

#Plot a heatmap of the motifs over pseudotime_
pseudo.df <- cbind(as.data.frame(sce$slingClusters), as.data.frame(sce$slingPseudotime_1))
...
motif_matrix$variance = apply(motif_matrix[,1:3480],1,var)

motif_matrix <- motif_matrix[order(motif_matrix$variance, decreasing = T),]
motif_matrix$counter <- 1:514
motif_matrix_top20 <- subset(motif_matrix, motif_matrix$counter <= 20)
motif_matrix_top20 <- motif_matrix_top20[,1:3480]
...
pheatmap(motif_matrix_top20[,c(pseudotime_order$index)], cluster_cols = F ,annotation_col = pseudotime_order[,c(1:2,4)], show_rownames=T, show_colnames = F,clustering_distance_rows = "correlation", clustering_method = "ward.D2", annotation_colors = ann_colors, scale = "column")


#Plot some motifs over the umap
colnames(x.sp3.sample_peaks_mmat_combined@mmat)[211]<-"MA0140.2_GATA1_TAL1"

 plotFeatureSingle( obj=x.sp3.sample_peaks_mmat_combined,feature.value=x.sp3.sample_peaks_mmat_combined@mmat[,'MA0841.1_NFE2'], method="umap",  main='MA0841.1_NFE2', point.size=0.4,  point.shape=19 , quantiles=c(0.2, 0.8) )

 plotFeatureSingle( obj=x.sp3.sample_peaks_mmat_combined,feature.value=x.sp3.sample_peaks_mmat_combined@mmat[,'MA0035.3_Gata1'], method="umap",  main='MA0035.3_Gata1', point.size=0.4,  point.shape=19 , quantiles=c(0.2, 0.8) )
 
 plotFeatureSingle( obj=x.sp3.sample_peaks_mmat_combined,feature.value=x.sp3.sample_peaks_mmat_combined@mmat[,'MA0795.1_SMAD3'], method="umap",  main='MA0795.1_SMAD3', point.size=0.4,  point.shape=19 , quantiles=c(0.2, 0.8) )
 
 plotFeatureSingle( obj=x.sp3.sample_peaks_mmat_combined,feature.value=x.sp3.sample_peaks_mmat_combined@mmat[,'MA0080.4_SPI1'], method="umap",  main='MA0080.4_SPI1', point.size=0.4,  point.shape=19 , quantiles=c(0.2, 0.8) )

 plotFeatureSingle( obj=x.sp3.sample_peaks_mmat_combined,feature.value=x.sp3.sample_peaks_mmat_combined@mmat[,'MA0140.2_GATA1_TAL1'], method="umap",  main='MA0140.2_GATA1_TAL1', point.size=0.4,  point.shape=19 , quantiles=c(0.2, 0.8) )
 
 plotFeatureSingle( obj=x.sp3.sample_peaks_mmat_combined,feature.value=x.sp3.sample_peaks_mmat_combined@mmat[,'MA0002.2_RUNX1'], method="umap",  main='MA0002.2_RUNX1', point.size=0.4,  point.shape=19 , quantiles=c(0.2, 0.8))
 
  plotFeatureSingle( obj=x.sp3.sample_peaks_mmat_combined,feature.value=x.sp3.sample_peaks_mmat_combined@mmat[,'MA0594.1_Hoxa9'], method="umap",  main='MA0594.1_Hoxa9', point.size=0.4,  point.shape=19 , quantiles=c(0.2, 0.8))
  
   plotFeatureSingle( obj=x.sp3.sample_peaks_mmat_combined,feature.value=x.sp3.sample_peaks_mmat_combined@mmat[,'MA0102.3_CEBPA'], method="umap",  main='MA0102.3_CEBPA', point.size=0.4,  point.shape=19 , quantiles=c(0.2, 0.8))
   
    plotFeatureSingle( obj=x.sp3.sample_peaks_mmat_combined,feature.value=x.sp3.sample_peaks_mmat_combined@mmat[,'MA0139.1_CTCF'], method="umap",  main='MA0139.1_CTCF', point.size=0.4,  point.shape=19 , quantiles=c(0.2, 0.8))
```
