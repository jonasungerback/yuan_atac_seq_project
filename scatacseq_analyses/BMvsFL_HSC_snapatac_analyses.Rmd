...
x.sp3.sample_peaks = scaleCountMatrix(obj=x.sp3.sample_peaks, cov=x.sp3.sample_peaks@metaData$promoter_ratio + 1,mat="gmat",method = "RPM")

# smooth the cell-by-gene matrix
x.sp3.sample_peaks = runMagic( obj=x.sp3.sample_peaks,input.mat="gmat",step.size=3)
par(mfrow = c(3, 3))

#Plot the marker genes above.    
for(i in 1:9){
  
    plotFeatureSingle( obj=x.sp3.sample_peaks,feature.value=x.sp3.sample_peaks@gmat[, marker.genes[i]], method="umap",  main=marker.genes[i], point.size=0.1,  point.shape=19, quantiles=c(0, 1) )
...
setwd("/mnt/data/common/jonas/scatac/191204_yuan_FLvsBM_HSC_BHHTF7DRXX/snap_atac_analysis/snap_atac_macs2_peaks_from_sample/homer_motifs_peaks_sample")
peaks.names = system("ls | grep narrowPeak.txt", intern=TRUE)
...
x.sp3.sample_peaks = createPmat(x.sp3, peak.gr,  do.par = TRUE, num.cores = 24)
x.sp3.sample_peaks = makeBinary(x.sp3.sample_peaks, mat="pmat")


save.image("/mnt/data/common/jonas/scatac/191204_yuan_FLvsBM_HSC_BHHTF7DRXX/snap_atac_analysis/HSC_snap_atac_analysis.RData")
```
