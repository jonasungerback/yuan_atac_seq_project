---
title: "bulk_atac_diff_peak_snap_atac_analysis"
...
setwd("/mnt/data/common/jonas/scatac/191204_yuan_FLvsBM_HSC_BHHTF7DRXX/snap_atac_analysis/diff_peak_from_bulk_atac/")
...
x.sp3.bulkatac_diff_peaks = createPmat(x.sp3.sample_peaks, peak.gr,  do.par = TRUE, num.cores = 4)
x.sp3.bulkatac_diff_peaks = makeBinary(x.sp3.bulkatac_diff_peaks, mat="pmat")
...
sce2$cell_type <- x.sp3.bulkatac_diff_peaks@sample
sce2$snap_cluster <- x.sp3.bulkatac_diff_peaks@cluster
plotReducedDim(sce2, use_dimred = "UMAP", colour_by = "cell_type")
plotReducedDim(sce2, use_dimred = "DiffusionMap", colour_by = "cell_type")
...
ggplot(as.data.frame(colData(sce2)), aes(x = sce2$slingPseudotime_1, y = cell_type, 
    ...
    ggtitle("Cells ordered by Slingshot pseudotime bulk diff peaks")

ggsave(filename = "slingshot_pseudotime_order_cell_type_bulk_diff_peaks.pdf", device = "pdf",path = "/mnt/data/common/jonas/scatac/191204_yuan_FLvsBM_HSC_BHHTF7DRXX/snap_atac_analysis/diff_peak_from_bulk_atac/", units = c("cm"), width = 20, height = 15)
...
ggsave(filename = "slingshot_pseudotime_order_snap_cluster_bulk_diff_peaks.pdf", device = "pdf",path = "/mnt/data/common/jonas/scatac/191204_yuan_FLvsBM_HSC_BHHTF7DRXX/snap_atac_analysis/diff_peak_from_bulk_atac/", units = c("cm"), width = 20, height = 15)
...
x.sp3.bulkatac_diff_peaks = makeBinary(x.sp3.bulkatac_diff_peaks, "pmat");

x.sp3.bulkatac_diff_peaks_mouse <- x.sp3.bulkatac_diff_peaks

#To get a complete list. Run this against both mouse and human and combine them in some way.
x.sp3.bulkatac_diff_peaks_mouse@mmat = runChromVAR(
    obj=x.sp3.bulkatac_diff_peaks_mouse,
    input.mat="pmat",
...
x.sp3.bulkatac_diff_peaks_human <- x.sp3.bulkatac_diff_peaks

x.sp3.bulkatac_diff_peaks_human@mmat = runChromVAR(
    obj=x.sp3.bulkatac_diff_peaks_human,
    input.mat="pmat",
    genome=BSgenome.Mmusculus.UCSC.mm10,
    min.count=10,
...
x.sp3.sample_peaks_mmat_combined <- x.sp3.bulkatac_diff_peaks_mouse
x.sp3.sample_peaks_mmat_combined@mmat <- cbind(x.sp3.bulkatac_diff_peaks_mouse@mmat,x.sp3.bulkatac_diff_peaks_human@mmat)
rm(x.sp3.bulkatac_diff_peaks_mouse)
rm(x.sp3.bulkatac_diff_peaks_human)
...
for (i in 1:length(colnames(x.sp3.sample_peaks_mmat_combined@mmat)) ) {
  ...
  ggsave(filename = paste(motif_i,"_diff_peaks_bulk_chromVAR_enrichment_per_sample.pdf",sep = "."), device = "pdf", dpi = 150, units = c("cm"), width = 10, height = 9)
}
