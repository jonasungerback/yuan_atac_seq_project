...
which(x.sp2@sample =="FL_HSC")
which(x.sp2@sample =="CD41neg")
x.sp3 = x.sp2[c( which(x.sp2@sample =="FL_HSC"),  which(x.sp2@sample =="CD41neg")), ];
...
plotViz(obj=x.sp3,method="umap",  main="FLHSCvsCD41neg", point.color=x.sp3@sample,  point.size=0.2, point.shape=19, point.alpha=0.8, text.add=TRUE,text.size=1.5, text.color="black", text.halo.add=TRUE, text.halo.color="white",text.halo.width=0.2,legend.add=FALSE)

plotViz(obj=x.sp3,method="umap",  main="FLHSCvsCD41neg", point.color=x.sp3@cluster,  point.size=0.2, point.shape=19, point.alpha=0.8, text.add=TRUE,text.size=1.5, text.color="black", text.halo.add=TRUE, text.halo.color="white",text.halo.width=0.2,legend.add=FALSE)
...
peaks.ls = mclapply(seq(clusters.sel.peaks), function(i){
    ...
    runMACS(
        ...
        output.prefix=paste0("FLHSCvsCD41_peaks_cluster.", gsub(" ", "_", clusters.sel.peaks)[i]),
...
peaks.sample = lapply(seq(samples.sel.peaks), function(i){
    ...
    runMACS(
        ...
        output.prefix=paste0("FLHSCvsCD41_peaks_sample.", gsub(" ", "_", samples.sel.peaks)[i]),
        path.to.snaptools="/opt/anaconda3/bin/snaptools",
        path.to.macs="/opt/anaconda3/bin/macs2",
        gsize="mm", 
        buffer.size=500, 
...
setwd("/mnt/data/common/jonas/scatac/191204_yuan_FLvsBM_HSC_BHHTF7DRXX/snap_atac_analysis/FLvsCD41neg_analysis/FLHSCvsCD41neg_peaks_cluster")
...
x.sp3.sample_peaks = createPmat(x.sp3, peak.gr,  do.par = TRUE, num.cores = 8)
...
save.image("/mnt/data/common/jonas/scatac/191204_yuan_FLvsBM_HSC_BHHTF7DRXX/snap_atac_analysis/FLvsCD41neg_analysis/FL_HSCvsCD41neg_snapatac_analyses.RData")
...
########## LOOKS LIKE A PEAK CLUSTERING AND TRAJECTORY ANALYSIS HERE WILL BE EXTREMELY DIFFICULT SO WILL MOVE ON WITH DAR AND MOTIF CALLING AND MAYBE REVISIT THIS LATER.
...
ggplot(as.data.frame(colData(sce2)), aes(x = sce2$slingPseudotime_1, y = cell_type, 
                              colour = cell_type)) +
...
ggsave(filename = "slingshot_pseudotime_order_cell_type_var_top10000_peaks.pdf", device = "pdf",path = "/mnt/data/common/jonas/scatac/191204_yuan_FLvsBM_HSC_BHHTF7DRXX/snap_atac_analysis/Snap_Figures/", units = c("cm"), width = 20, height = 15)
...
sample.in.cluster <- sample.in.cluster[,c(2,4)]

#colnames(sample.in.cluster) <- c("FL_HSC","CD41neg", "CD41pos", "MkP")
colnames(sample.in.cluster) <- c("FL_HSC","CD41neg")
rownames(sample.in.cluster) <- c("cluster1","cluster2","cluster3","cluster4","cluster5","cluster6","cluster7","cluster8","cluster9","cluster10")
...
ggsave(filename = "FLvsCD41neg_snap_atac_clusters_per_sample.pdf", device = "pdf",path = "/mnt/data/common/jonas/scatac/191204_yuan_FLvsBM_HSC_BHHTF7DRXX/snap_atac_analysis/FLvsCD41neg_analysis/", units = c("cm"), width = 20, height = 15)
...
ggsave(filename = "FLvsCD41neg_snap_atac_samples_per_cluster.pdf", device = "pdf",path = "/mnt/data/common/jonas/scatac/191204_yuan_FLvsBM_HSC_BHHTF7DRXX/snap_atac_analysis/FLvsCD41neg_analysis/", units = c("cm"), width = 20, height = 15)
...
for(cluster_i in levels(x.sp3.sample_peaks@cluster)){
  ...
  peak_name <- paste(cluster_i,"FLvsCD41_DARs.bed", sep = "_")
...
plot_order_clusters <- factor( c(rep(c(1), times=308) ,rep( c(2), times=451 ), rep( c(3), times=443 ),rep( c(4), times=801 ),rep( c(5), times=253 ),rep( c(6), times=449 ),rep( c(7), times=527 ),rep(c(8), times=341),rep(c(9), times=555),rep(c(10), times=544)), levels = c(1:10)  ) 
...
for (i in 1:length(colnames(x.sp3.sample_peaks_mmat_combined@mmat)) ) {
  ...
  ggsave(filename = paste(motif_i,"FLvsCD41neg_chromVAR_enrichment_per_cluster.pdf",sep = "."), device = "pdf", dpi = 150, units = c("cm"), width = 10, height = 9)
}
...
colnames(x.sp3.sample_peaks_mmat_combined@mmat)[211]<-"MA0140.2_GATA1_TAL1"
 ...
 plotFeatureSingle( obj=x.sp3.sample_peaks_mmat_combined,feature.value=x.sp3.sample_peaks_mmat_combined@mmat[,'MA0841.1_NFE2'], method="umap",  main='MA0841.1_NFE2', point.size=0.1,  point.shape=19 , quantiles=c(0, 0.9) )

 plotFeatureSingle( obj=x.sp3.sample_peaks_mmat_combined,feature.value=x.sp3.sample_peaks_mmat_combined@mmat[,'MA0035.3_Gata1'], method="umap",  main='MA0035.3_Gata1', point.size=0.1,  point.shape=19 , quantiles=c(0, 0.9) )
