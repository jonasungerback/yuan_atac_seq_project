...
# I must filter the barcodes. Can I use the ones filtered already in the cellranger filtering. 200520. For now use the filtering above
library(stringr)
...
data = data.frame(UMI=UMI, promoter_ratio=promoter_ratio, barcode=barcodes[,1])
...
x.sp2@metaData$barcode[25353:97306] <- paste(x.sp2@metaData$barcode[25353:97306], "ABM_CD41neg", sep = "_") 
x.sp2@barcode[25353:97306] <- paste(x.sp2@barcode[25353:97306], "ABM_CD41neg", sep = "_")

x.sp2@metaData$barcode[97307:122604] <- paste(x.sp2@metaData$barcode[97307:122604], "ABM_CD41pos", sep = "_") 
x.sp2@barcode[97307:122604] <- paste(x.sp2@barcode[97307:122604], "ABM_CD41pos", sep = "_")

x.sp2@metaData$barcode[122605:155679] <- paste(x.sp2@metaData$barcode[122605:155679], "ABM_MkP", sep = "_") 
x.sp2@barcode[122605:155679] <- paste(x.sp2@barcode[122605:155679], "ABM_MkP", sep = "_")
...
```{r re_clustering_no_MkP, echo=FALSE}
#This is done so the MkP cluster will not drench the signal for some genes.
########## Test to cluster the data without the MkPs
which(x.sp3@sample != "ABM_MkP")
x.sp3_no_MkP = x.sp3[1:5838, ];

x.sp3_no_MkP <- addBmatToSnap(x.sp3_no_MkP)
x.sp3_no_MkP = makeBinary(x.sp3_no_MkP, mat="bmat")
x.sp3_no_MkP = runDiffusionMaps(obj=x.sp3_no_MkP, input.mat="bmat", num.eigs=50)

plotDimReductPW(obj=x.sp3_no_MkP, eigs.dims=1:50,point.size=0.3, point.color="grey", point.shape=19,point.alpha=0.6, down.sample=5000, pdf.file.name=NULL,  pdf.height=7, pdf.width=7)
# A little tricky but choose the 10 first dimensions since that is a little blob shaped after.

#Graph based clusterning
x.sp3_no_MkP = runKNN(obj=x.sp3_no_MkP,eigs.dims=1:6,k=50) #We set it to 50 to make a more stringent clustering. More similar neigbours,
x.sp3_no_MkP=runCluster(obj=x.sp3_no_MkP,tmp.folder=tempdir(),louvain.lib="R-igraph",seed.use=10);
x.sp3_no_MkP@metaData$cluster = x.sp3_no_MkP@cluster

#Visualization
x.sp3_no_MkP = runViz(obj=x.sp3_no_MkP, tmp.folder=tempdir(),dims=2, eigs.dims=1:6, method="umap",seed.use=10)

par(mfrow = c(2, 2))

#Some cluster and qc-plotting.
plotViz(obj=x.sp3_no_MkP,method="umap",  main="HSCvsMkP", point.color=x.sp3_no_MkP@sample,  point.size=0.2, point.shape=19, point.alpha=0.8, text.add=TRUE,text.size=1.5, text.color="black", text.halo.add=TRUE, text.halo.color="white",text.halo.width=0.2,legend.add=FALSE)

x.sp3_no_MkP = addBmatToSnap(x.sp3_no_MkP);
x.sp3_no_MkP = createGmatFromMat(obj=x.sp3_no_MkP,  input.mat="bmat",genes=genes.sel.gr, do.par=TRUE, num.cores=24)
# normalize the cell-by-gene matrix
#x.sp3_no_MkP = scaleCountMatrix(obj=x.sp3_no_MkP, cov=x.sp3_no_MkP@metaData$passed_filters + 1,mat="gmat",method = "RPM")
x.sp3_no_MkP = scaleCountMatrix(obj=x.sp3_no_MkP, cov=x.sp3_no_MkP@metaData$promoter_ratio   + 1,mat="gmat",method = "RPM")
# smooth the cell-by-gene matrix
x.sp3_no_MkP = runMagic( obj=x.sp3_no_MkP,input.mat="gmat",step.size=3)
par(mfrow = c(3, 3))

#Plot the marker genes above.    
for(i in 1:9){
  
    plotFeatureSingle( obj=x.sp3_no_MkP,feature.value=x.sp3_no_MkP@gmat[, marker.genes[i]], method="umap",  main=marker.genes[i], point.size=0.1,  point.shape=19, quantiles=c(0, 1) )
...
```{r GREAT_analysis_of_CD41pos_or_CD41_neg_DARs, echo=FALSE}
...
library(rGREAT)
job = submitGreatJob(
    gr                    = x.sp3.sample_peaks@peak[idyCD41neg],
    bg                    = NULL,
    species               = "mm10",
...
library(cicero)
input_cds <- make_atac_cds(peaks.df, binarize = FALSE)
...
###################Skip this for now
#Can I pick these connections apart and annotate with my annotate2dbed tool. Lift it back in and only pull out interesting interactions.
library(stringr)
s <- str_split_fixed(conns$Peak1, "_",3)
conn_ap1 <- cbind(s[,1],s[,2],s[,3] )
colnames(conn_ap1) <-c("chr_1", "start_1", "end_1")

s <- str_split_fixed(conns$Peak2, "_",3)
conn_ap2 <- cbind(s[,1],s[,2],s[,3] )
colnames(conn_ap2) <-c("chr_2", "start_2", "end_2")

conns_df <- cbind(conn_ap1, conn_ap2, conns$coaccess)
colnames(conns_df)[7] <- "coaccess"
write.table(conns_df[,1:6], "cicero_connections.txt", sep = "\t", row.names = F, quote = F)
...
#Try this.
#Can I mege conns with fData(input_cds)
cicero_peaks <- as.data.frame(fData(input_cds))
#Remove NA peaks. 
cicero_peaks <- cicero_peaks[complete.cases(cicero_peaks), ]
#cicero_peaks$site_name <- as.character(cicero_peaks$site_name)


#See if the site exists in conns.
...
#conns_filt$Peak1 <- as.character(conns_filt$Peak1)
#conns_filt$Peak2 <- as.character(conns_filt$Peak2)

#Should be just the ones where one ap is the promoter that is interesting.
#conns_filt <- subset(conns_filt, conns_filt$ap1 == 1 & conns_filt$coaccess >= 0.03| conns_filt$ap2 == 1 & conns_filt$coaccess >= 0.03)
conns_filt <- subset(conns_filt, conns_filt$ap1 == 1 | conns_filt$ap2 == 1 )

test <- merge(conns_filt, cicero_peaks, by.x = "Peak1", by.y = "site_name", all.x = T)
test <- test[,c(1:5,11)]
colnames(test)[6] <- "TSS_1"

test2 <- merge(test, cicero_peaks, by.x = "Peak2", by.y = "site_name", all.x = T)
test2 <- test2[,c(1,2,4,5,6,12)]
colnames(test2)[6] <- "TSS_2"

test2 <- test2[,c(2,1,5,6)]

conns_filt <- test2

#Filter out Fluidigm genes from conns_filt:
fluidigm_genes <- read.delim("gene_list_fluidigm.txt", header = F)
marker.genes = as.vector(fluidigm_genes[,1])

#Filter for fluidigm genes.
...
#If TSS_1 matches, bring out coordinates from Peak2 plus gene name and vice versa.

peak2 <- cbind( as.data.frame(conns_filt_fluidigm[which(conns_filt_fluidigm$TSS_1 > 0),2]),as.data.frame(conns_filt_fluidigm[which(conns_filt_fluidigm$TSS_1 > 0),3]))
colnames(peak2) <- c("coord", "gene")

peak1 <- cbind( as.data.frame(conns_filt_fluidigm[which(conns_filt_fluidigm$TSS_2 > 0),1]),as.data.frame(conns_filt_fluidigm[which(conns_filt_fluidigm$TSS_2 > 0),4]))
colnames(peak1) <- c("coord", "gene")

int_conns_filt_fluidigm <- rbind(peak1,peak2)
int_conns_filt_fluidigm$coord <- as.character(int_conns_filt_fluidigm$coord)
...
colnames(int_conns_filt_fluidigm) <-c("chr", "start", "end", "gene")

#Transform to a GRanges object
...
#Run regions_to_av_access
test <- regions_to_av_access(marker.genes = marker.genes, full_peak_set=peak.gr, peak_mat=x.sp3.sample_peaks@pmat , grange_obj = int_conns_filt_fluidigm.gr)
View(do.call(rbind, test[[1]]))
...
plot_connections(conns, "chr2", 9763451, 9868598,
                 gene_model = gene_anno, 
                 coaccess_cutoff = .25, 
                 connection_width = .5, 
                 collapseTranscripts = "longest" )

plot_connections(conns, "chr1", 3105010, 3672890,
                                   gene_model = gene_anno, 
                                   coaccess_cutoff = .25, 
                                   connection_width = .5, 
                                   collapseTranscripts = "longest" )
...
cell_order <- as.data.frame(colnames(cicero_gene_activities))
s <- str_split_fixed(cell_order[,1], "-",2)
cell_order <-as.data.frame(s[,1])
colnames(cell_order)[1] <- "cell_order"

MkP_genes <- read.delim("MkP_geneset.grp", stringsAsFactors = F, header = F)

GAS_MkP_genes <- as(unnorm_ga, "matrix")
GAS_MkP_genes <- merge(MkP_genes,GAS_MkP_genes, by.x= "V1", by.y = "row.names", all.x = T)
rownames(GAS_MkP_genes) <- GAS_MkP_genes$V1
GAS_MkP_genes <- GAS_MkP_genes[,2:8824]
GAS_MkP_genes <-  GAS_MkP_genes[complete.cases(GAS_MkP_genes),]
colnames(GAS_MkP_genes) <- cell_order$cell_order
GAS_MkP_genes <- t(GAS_MkP_genes)
GAS_MkP_genes_cell_mean <- apply(GAS_MkP_genes,1,mean)
  

  
  dat = data.frame(x= rownames(GAS_MkP_genes), y=GAS_MkP_genes_cell_mean);
...
#Calculate the means
median(GAS_MkP_genes_cell_mean[which(rownames(GAS_MkP_genes)=="FL_HSC")]) #8.24394e-05
median(GAS_MkP_genes_cell_mean[which(rownames(GAS_MkP_genes)=="CD41neg")]) #7.238255e-05
median(GAS_MkP_genes_cell_mean[which(rownames(GAS_MkP_genes)=="CD41pos")]) #7.817406e-05
median(GAS_MkP_genes_cell_mean[which(rownames(GAS_MkP_genes)=="MkP")]) #9.079765e-05
```
...
```{r make_a_score_of_MkP_genes_from_normalized_gene_marker_matrix, echo=FALSE}
system("wget http://renlab.sdsc.edu/r3fang/share/github/Mouse_Brain_10X/gencode.vM16.gene.bed");
genes = read.table("../gencode.vM16.gene.bed");
genes.gr = GRanges(genes[,1],  IRanges(genes[,2]-100000, genes[,3]+100000), name=genes[,4]
  );
#Read in MkP genes
MkP_genes <- read.delim("../MkP_geneset.grp", stringsAsFactors = F)

marker.genes = as.vector(MkP_genes[,1])
genes.sel.gr <- genes.gr[which(genes.gr$name %in% marker.genes)]
# Add a pmat based on these regions that later will be TF-IFD normalized.
rmPmatFromSnap(x.sp3.ext_peaks)
x.sp3.ext_peaks = createPmat(x.sp3.ext_peaks, genes.sel.gr,  do.par = TRUE, num.cores = 1)
#Lif out the matrix:
peaks_Mk_genes <- as(object = x.sp3.ext_peaks@pmat, Class = 'matrix')

#TF-IFD-normalize the matrix
norm.mat_peaks_Mk_genes <- t(TIFD_zscore_norm(peaks_Mk_genes, tfid_norm = 1))
#Calculate total accessibility per cell for these regions
norm.peaks_Mk_genes <- as.data.frame( apply(norm.mat_peaks_Mk_genes,1,sum, na.rm=T))

#Caluclate the totalaccessibility for all peaks
norm.mat <- as.data.frame( apply(norm.pmat,1,sum, na.rm=T))
#Calculate enrichment in marker genes over total enrichment
peaks_Mk_genes_enriched <- peaks_Mk_genes_enriched/norm.mat


peaks_Mk_genes_df <- cbind( as.data.frame(x.sp3.ext_peaks@sample), peaks_Mk_genes_enriched, norm.peaks_Mk_genes)


colnames(peaks_Mk_genes_df) <- c("sample", "peaks_Mk_genes_enriched", "norm.peaks_Mk_genes")

plot_order_samples <- factor(c(rep(c('FL_HSC'), times=2403), rep(c('CD41neg'), times=2310), rep(c('CD41pos'), times=1123), rep(c('MkP'), times=2986)), levels=(c('FL_HSC', 'CD41neg', 'CD41pos', 'MkP')))

#Up_enriched
ggplot()+geom_violin(data = peaks_Mk_genes_df, aes( y = peaks_Mk_genes_enriched, x = sample, fill = sample),draw_quantiles = c(0.25, 0.5, 0.75)) + theme_bw() + ggtitle("") + xlab("Sample") + ylab("Peak subset/total peak accessibility") 
```
